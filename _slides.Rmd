...
data.frame(
  ...
  slope_C = c(0,0,0,0, 0,0,0,0, 1,2,3,4)
...
case[[1]] %>% relocate(mt, phase) %>% kable() %>% kable_classic_2()
...
res_1 <- plm(case)
res_2 <- plm(case, AR = 3)
out <- summary(res_1$full.model)$coefficients 
...
```{r}
out <- summary(res_2$full.model)$tTable 
rownames(out) <- c("Intercept", "Trend", "Level B", "Slope B")
out %>% kable() %>% kable_classic_2()
```
